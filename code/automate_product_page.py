...
def prompt_gpt(prompt):
    ...
    try:
        final_prompt = (
            "KEEP THE GENERATED CONTENT SHORT AND CONVINCING. ONLY EFFECTIVE AND IMPORTANT CONTENT.DO NOT GENERATE LONG CONTENT."
            + prompt
        )
        api_key = os.getenv("OPENAI_API_KEY")
        if not api_key:
...
def generate_product_feature_prompt(
    original_feature, product_title, product_description, language
):
...
def generate_social_proof_prompt(
    original_text, product_title, product_description, language
):
...
def generate_collapsible_content_prompt(
    original_content, product_title, product_description, language
):
...
def generate_full_review_prompt(
    original_review_head,
...
def generate_feature_list_item_prompt(
    original_title, product_title, product_description, language
):
...
def generate_faq_prompt(
    original_question, original_answer, product_title, product_description, language
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Translate and generate product page content"
    )
    parser.add_argument(
        "--language", "-l", required=True, help="Target language (e.g., fr, es, de)"
    )
    parser.add_argument(
        "--brand", "-b", required=True, help="Brand name (e.g., GlamCurl)"
    )
    parser.add_argument("--product", "-p", required=True, help="Product title")
    parser.add_argument(
        "--description", "-d", required=True, help="Product description"
    )
    ...
    prompt = generate_product_tagline_prompt(
        "Investera i ett stylingverktyg som kan göra allt !",
        product_title,
        product_description,
        language,
    )
    ...
    for placeholder, original_text in feature_tasks.items():
        prompt = generate_product_feature_prompt(
            original_text, product_title, product_description, language
        )
    ...
    quantity_tasks = {
        ...
        "NEW_QUANTITY_G3H4_GENERATED": (
            "Buy 2, Get 1 FREE",
            generate_quantity_option_prompt,
        ),
        "NEW_QUANTITY_I5J6_GENERATED": (
            "Buy 3, Get 2 FREE",
            generate_quantity_option_prompt,
        ),
        "NEW_QUANTITY_K7L8_GENERATED": (
            "SPECIAL OFFER - Limited Time",
            generate_badge_text_prompt,
        ),
        "NEW_QUANTITY_M9N0_GENERATED": (
            "FLASH SALE — Grab It Before It's Gone",
            generate_badge_text_prompt,
        ),
        "NEW_QUANTITY_O1P2_GENERATED": (
            "6 Months Supply | FREE Shipping",
            generate_unit_label_prompt,
        ),
        "NEW_QUANTITY_Q3R4_GENERATED": (
            "12 Months Supply | FREE Shipping",
            generate_unit_label_prompt,
        ),
        "NEW_QUANTITY_S5T6_GENERATED": (
            "Save (%) - GET ONE FREE",
            generate_save_text_prompt,
        ),
        "NEW_QUANTITY_U7V8_GENERATED": (
            "Save (%) - GET TWO FREE",
            generate_save_text_prompt,
        ),
    }
    ...
    prompt = generate_promo_html_prompt(
        "<strong>12</strong> Months Supply | <strong>FREE Shipping</strong>",
        product_title,
        language,
    )
    ...
    prompt = generate_social_proof_prompt(
        "<p><strong>50+ certifierade</strong> hudläkare rekommenderar detta till sina patienter.</p>",
        product_title,
        product_description,
        language,
    )
    ...
    for placeholder, original_text in collapsible_tasks.items():
        prompt = generate_collapsible_content_prompt(
            original_text, product_title, product_description, language
        )
        result = prompt_gpt(prompt)
        replace_in_file(PRODUCT_JSON_PATH, placeholder, result)

    prompt = generate_guarantee_prompt(
        "30 dagars pengar tillbaka garanti",
        "<p>Om du inte är nöjd med din upplevelse...</p>",
        product_title,
        language,
    )
    result = prompt_gpt(prompt)
    if result and "|" in result:
        ...
        replace_in_file(
            PRODUCT_JSON_PATH, "NEW_COLLAPSIBLE_TAB_C5D6_GENERATED", title.strip()
        )
        replace_in_file(
            PRODUCT_JSON_PATH, "NEW_COLLAPSIBLE_TAB_I1J2_GENERATED", desc.strip()
        )

    # --- Other Sections ---
    prompt = generate_product_tagline_prompt(
        "<strong>Se locktång i aktion !</strong>",
        product_title,
        product_description,
        language,
    )
    ...
    prompt = generate_product_tagline_prompt(
        "<strong>Sublima Lockar med locktång</strong>",
        product_title,
        product_description,
        language,
    )
    ...
    prompt = generate_collapsible_content_prompt(
        "<p>Tänk dig att varje morgon börja med perfekta lockar...</p>",
        product_title,
        product_description,
        language,
    )
    ...
    lumin_slider_tasks = {
        "NEW_LUMIN_SLIDER_Q9R0_GENERATED": (
            "Se hur andra älskar sin Locktång !",
            generate_product_tagline_prompt,
        ),
        "NEW_LUMIN_SLIDER_S1T2_GENERATED": (
            "Äkta recensioner från riktiga människor",
            generate_product_tagline_prompt,
        ),
        "NEW_LUMIN_SLIDER_U3V4_GENERATED": (
            "Köp din Locktång idag",
            generate_product_tagline_prompt,
        ),
        "NEW_LUMIN_SLIDER_W5X6_GENERATED": (
            "Betygsatt 4,8/5 av 14 428+ glada kunder",
            generate_social_proof_prompt,
        ),
    }
    ...
    prompt = generate_product_tagline_prompt(
        "Kvalitet som du inte hittar någon annanstans !",
        product_title,
        product_description,
        language,
    )
    ...
    prompt = generate_product_tagline_prompt(
        "Ledande konkurrent", product_title, product_description, language
    )
    result = prompt_gpt(prompt)
    ...
    prompt = generate_product_tagline_prompt(
        "Varje morgon, perfekta lockar", product_title, product_description, language
    )
    result = prompt_gpt(prompt)
    ...
    prompt = generate_collapsible_content_prompt(
        "<p>Vi stödjer ett medvetet och ansvarsfullt förhållningssätt...</p>",
        product_title,
        product_description,
        language,
    )
    ...
    prompt = generate_product_tagline_prompt(
        "Varför locktång?", product_title, product_description, language
    )
    result = prompt_gpt(prompt)
    ...
    lumin_mega_features = {
        (
            "NEW_LUMIN_MEGA_FEATURE_1_GENERATED_TITLE",
            "NEW_LUMIN_MEGA_FEATURE_1_GENERATED_TEXT",
        ): (
            "6 Temperaturinställningar",
            "<p>Justera temperaturen för perfekta lockar...</p>",
        ),
        (
            "NEW_LUMIN_MEGA_FEATURE_2_GENERATED_TITLE",
            "NEW_LUMIN_MEGA_FEATURE_2_GENERATED_TEXT",
        ): (
            "<strong>Justerbar Timer</strong>",
            "<p>Skapa lösa eller täta lockar...</p>",
        ),
        (
            "NEW_LUMIN_MEGA_FEATURE_3_GENERATED_TITLE",
            "NEW_LUMIN_MEGA_FEATURE_3_GENERATED_TEXT",
        ): ("<strong>Snabb Laddning</strong>", "<p>Ladda på bara 4 timmar...</p>"),
        (
            "NEW_LUMIN_MEGA_FEATURE_4_GENERATED_TITLE",
            "NEW_LUMIN_MEGA_FEATURE_4_GENERATED_TEXT",
        ): (
            "<strong>Lång Batteritid</strong>",
            "<p>Upp till 50 minuters trådlös användning...</p>",
        ),
    }
    for (title_ph, text_ph), (
        original_title,
        original_text,
    ) in lumin_mega_features.items():
        prompt = generate_feature_list_item_prompt(
            original_title, product_title, product_description, language
        )
        result = prompt_gpt(prompt)
        if result and "|" in result:
    ...
    prompt = generate_product_tagline_prompt(
        "Skäm bort ditt hår med mjukhet", product_title, product_description, language
    )
    result = prompt_gpt(prompt)
    ...
    faqs = [
        (
            "Är denna locktång lämplig för alla hårtyper?",
            "<p>Absolut! Tack vare sina flera temperaturinställningar...",
        ),
        (
            "Kan man justera temperaturen?",
            "<p>Absolut! Den här locktång erbjuder <strong>sex...",
        ),
        (
            "Är locktång farlig för håret?",
            "<p>Nej, tack vare den <strong>keramiska turmalinen</strong>...",
        ),
        (
            "Fungerar det med kort hår?",
            "<p>Ja, så länge locken är tillräckligt lång för att...",
        ),
        ("Hur man rengör järnet?", "<p>För att rengöra locktång, stäng av den och..."),
        (
            "Är locktång transportabel och lätt att ta med?",
            "<p>Ja, denna locktång är <strong>transportabel</strong>...",
        ),
        (
            "Är materialen i locktången verkligen ekologiska?",
            "<p>Ja, vår locktång är tillverkad av hållbara och...",
        ),
        (
            "Hur fungerar frakt och returer?",
            "<p>Vi erbjuder snabb och pålitlig frakt...",
        ),
    ]
    faq_placeholders = [
        ("NEW_FAQ_{}_GENERATED_QUESTION", "NEW_FAQ_{}_GENERATED_ANSWER")
        for i in range(1, 9)
    ]

    for i, (q_ph, a_ph) in enumerate(faq_placeholders):
        original_q, original_a = faqs[i]
        prompt = generate_faq_prompt(
            original_q, original_a, product_title, product_description, language
        )
        result = prompt_gpt(prompt)
        if result and "|" in result:
    ...
    # Reviews
    prompt = generate_social_proof_prompt(
        "<strong>14428+</strong> Verkliga recensioner, verkliga resultat från <strong>människor som du.</strong>",
        product_title,
        product_description,
        language,
    )
    ...
    reviews = [
        (
            "Val av temperatur för att inte bränna håret",
            "Locktång fyller perfekt sin mission...",
        ),
        ("IMPRESIONANTE !", "Jag har rakt hår och min dröm är att ha lockigt..."),
        (
            "Pålitlig locktång med enkel hantering",
            "Jag har använt den automatiska locktången ett tag nu...",
        ),
        (
            "Fantastisk produkt, bra kvalitet",
            "Jag är supernöjd med denna sladdlösa locktång!...",
        ),
        (
            "Ottimo… lika effektiv som de stora märkena",
            "Presterar bra… systemet är väldigt enkelt...",
        ),
        ...
        (
            "🎁 Perfekt present… och billig",
            "Jag behövde en liten present till min tonåriga systerdotter...",
        ),
    ]
    review_placeholders = [
        ("NEW_REVIEW_{}_GENERATED_HEAD", "NEW_REVIEW_{}_GENERATED_TEXT")
        for i in range(1, 9)
    ]

    for i, (head_ph, text_ph) in enumerate(review_placeholders):
        original_head, original_text = reviews[i]
        prompt = generate_full_review_prompt(
            original_head, original_text, product_title, product_description, language
        )
        result = prompt_gpt(prompt)
        if result and "|" in result:
